/* ============================================================
   CYCLING MANAGER v0.25 · GRAND TOUR STAGE LAB INTEGRATION
...
const CM_STAGE_LAB_VERSION = "v0.25-stage-lab";
...
// Captured after the legacy GPX engine has enriched the Tour. This means the
// "continue with existing route" option also preserves the previous real Tour
// tracks instead of rolling back to the fictitious v0.24 profile.
const CM_STAGE_LAB_BASE_STAGES = Object.fromEntries(
  RACES.map(race => [race.id, clone(race.stages)])
);
    ...
    <div class="stage-lab-manager-shell">
      ...
      <section class="panel stage-lab-manager-info">
        <div>
          ...
          <span>Al pulsar <b>Usar GPX en Cycling Manager</b>, las etapas generadas sustituirán únicamente la geometría, altimetría y sectores del evento actual. Corredores, CP/W′, IA, contratos, material, nutrición, tácticas, clasificaciones y persistencia v0.24+ se conservan.</span>
        </div>
...
function restoreBaseRaceStages(raceId) {
  ...
  const base = CM_STAGE_LAB_BASE_STAGES[raceId];
...
function stageLabTypeToManager(labType, baseType) {
  // The calendar blueprint remains authoritative for race mechanics. Stage Lab
  // changes the real geometry and altitude, not the sporting format (pavé,
  // road stage, CRI or CRE) selected by the original event.
  if (["flat", "hilly", "mountain", "cobbles", "tt", "ttt"].includes(baseType)) return baseType;
  const map = {
    flat: "flat",
    rolling: "hilly",
    punchy: "hilly",
...
function convertStageLabTourForRace(race, tour) {
  ...
  const baseStages = CM_STAGE_LAB_BASE_STAGES[race.id] || race.stages;
...
function acceptStageLabTour(eventId, tour) {
  ...
  setTimeout(() => {
    try {
      ...
      CM_STAGE_LAB_ACCEPTING = false;
      __cm_stage_lab_prepareRosterSelection();
      v024PlusPersist?.(false);
      toast(`${race.name}: GPX de Stage Lab integrados en el motor físico`);
    } catch (error) {
...
init = function() {
  // Restore mutable race definitions before starting a completely new game.
  Object.entries(CM_STAGE_LAB_BASE_STAGES).forEach(([raceId, stages]) => {
    const race = byId(RACES, raceId);
    if (race) race.stages = clone(stages);
  });
  __cm_stage_lab_init();
  Game.stageLab = {
    version: CM_STAGE_LAB_VERSION,
...
loadGame = function() {
  const result = __cm_stage_lab_loadGame();
  restoreActiveStageLabRace();
  // The v0.24+ loader renders before returning. Re-render once the route data
  // has been reapplied to the mutable RACES catalogue.
...
renderHome = function() {
  ...
  if (title) title.textContent = "Cycling Manager Tour v0.25";
  const subtitle = app.querySelector(".header p");
  if (subtitle) subtitle.textContent = "Grand Tour Stage Lab para todo el calendario · GPX dinámicos · física CP/W′ cada 250 m";
  app.querySelectorAll(".race-card").forEach(card => {
    if (card.querySelector(".stage-lab-calendar-badge")) return;
    card.insertAdjacentHTML("beforeend", `<span class="badge green stage-lab-calendar-badge">STAGE LAB · GPX configurable</span>`);
  });
};
