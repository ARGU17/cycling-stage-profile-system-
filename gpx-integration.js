...
renderHome = function() {
  ...
  if (subtitle) subtitle.textContent = "Motor físico CP/W′ · etapas Tour sobre GPX real · perfil Radial interactivo · grupos sobre el recorrido";
  [...app.querySelectorAll(".race-card")].forEach(card => {
    if (!card.textContent.includes("Tour de France") || card.querySelector(".gpx-home-badge")) return;
    const gpxOnly = card.textContent.includes("GPX reales");
    card.insertAdjacentHTML(
      "beforeend",
      `<span class="badge green gpx-home-badge">${gpxOnly ? "100% GPX · 18 etapas" : "18/21 etapas con GPX"}</span>`
    );
  });
};
